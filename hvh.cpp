...
void HVH::GetAntiAimDirection ( ) {
	// edge aa.
	if ( g_menu.main.antiaim.edge.get ( ) && ( !g_menu.main.antiaim.manual_aa.get ( ) || side == 0.f ) && g_cl.m_local->m_vecVelocity ( ).length ( ) < 320.f ) {
		ang_t ang;
...
void HVH::DoRealAntiAim ( ) {
	...
	if ( g_menu.main.antiaim.manual_aa.get ( ) && m_aa_side != 0.f )
		g_cl.m_cmd->m_view_angles.y -= m_aa_side;
