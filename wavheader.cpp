...
bool sound::AdjustVolume ( WavHeader &header, float volume, float volRatio = 1.0f ) {
	// We can only adjust PMC (Uncompressed) WAV files
	if ( header.format_type != 1 )
		return false;


	float finalval = volume * volRatio;
	float finalVolume = std::clamp < float > ( finalval, 0.f, 1.f );
